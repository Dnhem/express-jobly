...
class Company {
  ...
  static async findAll(qString) {
    ...
    if (isQstringEmpty(query)) {
      query = `${query}
               ORDER BY name;`;
    } else if (qString.minEmployees) {
      query = `${query} WHERE num_employees >= ${+qString.minEmployees};`;
    } else if (qString.maxEmployees) {
      query = `${query} WHERE num_employees <= ${+qString.maxEmployees};`;
    } else if (qString.name) {
      query = `${query} WHERE LOWER(name) LIKE '%${qString.name}%';`;
    }
